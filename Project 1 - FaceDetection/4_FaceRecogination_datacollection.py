...
while True:
    ...
    # detect only the largest face - one face out of many : Closest Face 
    face_cord = face_detector.detectMultiScale(frame,1.3,6) 
    face_cord = sorted(face_cord, key = lambda x : x[2]*x[3]) # sorted by rectangular area
    # going only for the largest Face - using loop because face_cord can also be empty when no face is detected.
    for x,y,w,h  in face_cord[-1:]:
        ...
        padding = 10
        face_section = frame[y-padding:y+h+padding, x-padding:x+padding+w]	# cropping the frame
